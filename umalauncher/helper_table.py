...
class TrainingPartner:
    def __init__(self, partner_id, starting_bond, chara_info):
        ...
        if partner_id < 100:
            support_id = chara_info["support_card_array"][partner_id - 1][
                "support_card_id"
            ]
            ...
            try:
                chara_id = mdb.get_single_mode_unique_chara_dict()[
                    chara_info["scenario_id"]
                ][partner_id]
                self.chara_id = chara_id
                self.img = f"https://gametora.com/images/umamusume/characters/icons/chr_icon_{chara_id}.png"
            except KeyError:
                ...
                logger.error(
                    f"Could not find unique chara_id for partner_id {partner_id} in scenario {chara_info['scenario_id']}"
                )
    ...
    def add_effect_bonus_bond(self, bond):
        ...
        if self.partner_id <= 6 and 8 in self.chara_info.get(
            "chara_effect_id_array", []
        ):
            bond += 2

        # Add 2 extra bond when rising star is active and the partner is Akikawa
        elif self.partner_id == 102 and 9 in self.chara_info.get(
            "chara_effect_id_array", []
        ):
            bond += 2
    ...
    def calc_bonds(self):
        ...
        if self.partner_id < 1000:
            ...
            if self.partner_id <= 6:
                ...
                support_card_type = constants.SUPPORT_CARD_TYPE_DICT[
                    (support_card_data[1], support_card_data[2])
                ]
        ...
        return (
            max(bond, 0),
            max(self.calc_useful_bond(bond, self.starting_bond), 0),
            max(hint_bond, 0),
            max(self.calc_useful_bond(hint_bond, self.starting_bond + bond), 0),
        )
    ...
    def calc_useful_bond(self, amount, starting_bond):
        ...
        if self.partner_id <= 6:
            ...
            if support_card_id in (10094, 30160) and self.chara_info["scenario_id"] in (
                6,
            ):  # Only count Mei in Project L'Arc
                usefulness_cutoff = 60
            elif support_card_id in (10104, 30188) and self.chara_info[
                "scenario_id"
            ] in (
                7,
            ):  # Only count Ryoka in UAF
                usefulness_cutoff = 60
            else:
                support_card_data = mdb.get_support_card_dict()[support_card_id]
                support_card_type = constants.SUPPORT_CARD_TYPE_DICT[
        ...
        if 6 < self.partner_id <= 1000:
            if self.partner_id in (102,) and not self.chara_info["scenario_id"] in (
                6,
            ):  # Disable Akikawa usefulness in certain scenarios
                usefulness_cutoff = 60
            else:
...
class HelperTable:
    ...
    def __init__(self, carrotjuicer):
        ...
        self.preset_dict, self.selected_preset = (
            self.carrotjuicer.threader.settings.get_helper_table_data()
        )
    ...
    def update_presets(self, preset_dict, selected_preset):
        ...
        if (
            self.carrotjuicer.last_helper_data
            and self.carrotjuicer.browser
            and self.carrotjuicer.browser.alive()
        ):
            self.carrotjuicer.update_helper_table(self.carrotjuicer.last_helper_data)

    def create_helper_elements(self, data, last_data, sharp_1, sharp_2) -> str:
            ...
            if key in data and "command_info_array" in data[key]:
                for command in data[key]["command_info_array"]:
                    if "params_inc_dec_info_array" in command:
                        all_commands[command["command_id"]][
                            "params_inc_dec_info_array"
                        ] += command["params_inc_dec_info_array"]
        ...
        if "live_data_set" in data:
            for command in data["live_data_set"]["command_info_array"]:
                ...
                ] = command["performance_inc_dec_info_array"]
        ...
        if "arc_data_set" in data:
            ...
            for command in data["arc_data_set"].get("command_info_array", []):
                if command["command_id"] in all_commands:
                    all_commands[command["command_id"]]["add_global_exp"] = command[
                        "add_global_exp"
                    ]
            ...
            if 3 <= turn < 37 or 44 <= turn < 61:
                ...
                all_commands["ss_match"] = {
                    "command_id": "ss_match",
                    "params_inc_dec_info_array": data["arc_data_set"]
                    .get("selection_info", [])
                    .get("params_inc_dec_info_array", [])
                    + data["arc_data_set"]
                    .get("selection_info", [])
                    .get("bonus_params_inc_dec_info_array", []),
                }
        ...
        # Support Dict
        eval_dict = {
            eval_data["training_partner_id"]: TrainingPartner(
                eval_data["training_partner_id"],
        ...
        for command in all_commands.values():
            ...
            gained_stats = {
                stat_type: 0 for stat_type in set(constants.COMMAND_ID_TO_KEY.values())
            }
            gained_skillpt = 0
            total_bond = 0
            useful_bond = 0
            ...
            for param in command.get("params_inc_dec_info_array", []):
                if param["target_type"] < 6:
                    gained_stats[
                        constants.TARGET_TYPE_TO_KEY[param["target_type"]]
                    ] += param["value"]
            ...
            if command["command_id"] == "ss_match":
                ...
                arc_eval_dict = {
                    partner_data["chara_id"]: partner_data["target_id"]
                    for partner_data in data["arc_data_set"]["evaluation_info_array"]
                }

                for chara in data["arc_data_set"]["selection_info"][
            ...
            if "venus_data_set" in data:
                ...
                if (
                    len(data["venus_data_set"]["venus_spirit_active_effect_info_array"])
                    > 0
                    and data["venus_data_set"]["venus_spirit_active_effect_info_array"][
                        0
                    ]["chara_id"]
                    == 9041
                ):
            ...
            for training_partner_id in command.get("training_partner_array", []):
                ...
                if training_partner_id <= 6:
                    ...
                    if (
                        support_card_type != "friend"
                        or support_id == 30160
                        and scenario_id in (6,)
                        or support_id == 30052
                        and scenario_id in (3,)
                        or support_id == 30188
                        and support_id in (7,)
                    ):
                    ...
                    if (
                        support_card_type not in ("group", "friend")
                        and training_partner.starting_bond >= 80
                        and command["command_id"]
                        in constants.SUPPORT_TYPE_TO_COMMAND_IDS[support_card_type]
                    ):
                        rainbow_count += 1
                    elif (
                        support_card_type == "group"
                        and util.get_group_support_id_to_passion_zone_effect_id_dict()[
                            support_id
                        ]
                        in data["chara_info"]["chara_effect_id_array"]
                    ):
                        rainbow_count += 1
                    elif (
                        support_card_type != "friend"
                        and "venus_data_set" in data
                        and len(
                            data["venus_data_set"][
                                "venus_spirit_active_effect_info_array"
                            ]
                        )
                        > 0
                        and data["venus_data_set"][
                            "venus_spirit_active_effect_info_array"
                        ][0]["chara_id"]
                        == 9042
                    ):
                    ...
                    training_partner_id > 1000
                ):  # TODO: Maybe 1000 < training_partner_id < 9000
                    useful_partner_count += 1
            ...
            current_stats = data["chara_info"].get(
                constants.COMMAND_ID_TO_KEY[command["command_id"]], 0
            )
            ...
            if "live_data_set" in data:
                for token_data in command["performance_inc_dec_info_array"]:
                    gl_tokens[
                        constants.GL_TOKEN_LIST[token_data["performance_type"] - 1]
                    ] += token_data["value"]
            ...
            if "arc_data_set" in data:
                ...
                arc_eval_dict = {
                    partner_data["target_id"]: partner_data["chara_id"]
                    for partner_data in data["arc_data_set"]["evaluation_info_array"]
                }

                for arc_chara_id in [
                    arc_eval_dict[partner_id]
                    for partner_id in command.get("training_partner_array", [])
                ]:
                    if arc_chara_id in arc_charas:
                        arc_chara = arc_charas[arc_chara_id]
                        arc_gauge_gain += min(
                            1 + rainbow_count, 3 - arc_chara["rival_boost"]
                        )  # TODO: Try to avoid doing this right after a match is done?

                # Override row data for SS Match
                if command["command_id"] == "ss_match":
                    # Partners
                    rival_dict = {
                        rival["chara_id"]: rival
                        for rival in data["arc_data_set"]["arc_rival_array"]
                    }
                    selection_list = data["arc_data_set"]["selection_info"][
                        "selection_rival_info_array"
                    ]
            ...
            if "sport_data_set" in data:
                ...
                uaf_sport_rank = {
                    item["command_id"]: item["sport_rank"] for item in sport_levels
                }
                uaf_sport_compeition_win = data["sport_data_set"].get(
                    "competition_result_array", []
                )

                uaf_active_effects = data["sport_data_set"].get(
                ...
                for competition in uaf_sport_compeition_win:
                    if competition.get("result_state") == 1:
                        for win_command_id in competition.get(
                            "win_command_id_array", []
                        ):
                ...
                uaf_sport_competition = (
                    f"{group_counts['1']}/{group_counts['2']}/{group_counts['3']}"
                )

                uaf_consultations_left = len(
                    data["sport_data_set"].get("item_id_array", [])
                )
                ...
                for command_id, rank in uaf_sport_rank.items():
                    base = command_id - (
                        command_id % 100
                    )  # Get the base (2100, 2200, 2300, etc.)
                ...
                uaf_sport_gain = {
                    command_id: gain_rank for command_id, gain_rank in gain_info_list
                }
            ...
            for race_data in data["reserved_race_array"][0]["race_array"]:
                ...
                if not program_data:
                    util.show_warning_box(
                        f"Could not get program data for program_id {race_data['program_id']}"
                    )
                    continue

                if program_data["base_program_id"] != 0:
                    program_data = mdb.get_program_id_data(
                        program_data["base_program_id"]
                    )
        ...
        if self.carrotjuicer.threader.settings[
            "training_helper_table_scenario_presets_enabled"
        ]:
            scenario_preset = self.carrotjuicer.threader.settings[
                "training_helper_table_scenario_presets"
            ].get(str(scenario_id), None)
            if scenario_preset and self.selected_preset.name != scenario_preset:
                self.selected_preset = (
                    self.carrotjuicer.threader.settings.get_preset_with_name(
                        scenario_preset
                    )
                )
        else:
            general_preset = self.carrotjuicer.threader.settings[
                "training_helper_table_preset"
            ]
